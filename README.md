# GitHub Action to deploy an MkDocs(Material theme) site to GitHub Pages

![GitHub Badge](https://github.com/H-Sofie/mkdocs-deploy-with-material-theme/workflows/Build/badge.svg)

This GitHub action will deploy your [MkDocs site](https://www.mkdocs.org/) as [GitHub Pages](https://pages.github.com/), using the [Material](https://github.com/squidfunk/mkdocs-material) theme. You dont need to manually install `mkdocs` and `mkdocs-material`; by default, the actions will automatically install the latest versions of both. If you need to specify a particular version, please include it in the `requirements.txt` file following the appropriate format.

It assumes that an `mkdocs.yml` file is present in the top-level directory and the source files (Markdown, etc.) are in the `docs/` folder.
...
Some Python packages require C bindings. These packages can be installed using the `EXTRA_PACKAGES` variable. The `EXTRA_PACKAGES` variable will be passed to the `apt install` command of ubuntu before running `pip install` to install the Python packages.
